...
def metric_pop_report(data_rank, target_items, pop_dict, ts_item, topk=10, aug_pop=0, args=None):
    """
    ...
    # 安全地获取item popularity，处理越界情况
    target_items_safe = target_items.astype("int64")
    max_pop_index = len(pop_dict) - 1
    
    # 将越界的item ID设置为0（通常是padding或未知item）
    target_items_safe = np.clip(target_items_safe, 0, max_pop_index)
    item_pop = pop_dict[target_items_safe]
    
    count_s = len(item_pop[item_pop<ts_tail+aug_pop])
    count_l = len(item_pop[item_pop>=ts_tail+aug_pop])

    for i, rank in enumerate(data_rank):
        # 跳过padding index或越界的情况
        if i >= len(item_pop) or target_items[i] > max_pop_index:
            continue
...
def metric_pop_5group(pred_rank, 
    ...
    for i, rank in enumerate(pred_rank):
        # 跳过padding index或越界的情况
        if i >= len(target_items) or target_items[i] > max_pop_index:
            continue
            
        # 将越界的item ID设置为0（通常是padding或未知item）
        item_id = int(np.clip(target_items[i], 0, max_pop_index))
        target_pop = pop_dict[item_id]
    ...
    count = np.zeros(5)
    # 安全地处理target_items
    target_items_clipped = np.clip(target_items_safe, 0, max_pop_index)
    pop = pop_dict[target_items_clipped]
...
def metric_pop_5group_dict(pred_rank, 
                          pop_dict, 
                          target_items, 
                          thresholds=[10, 30, 60, 100], 
                          topk=10):
    """
    使用5组流行度分析的包装函数，返回字典格式结果
    """
    HR, NDCG, count = metric_pop_5group(pred_rank, pop_dict, target_items, thresholds, topk)
    
    # 定义组别名称（按交互次数分段）
    # 对应分段：1-9, 10-29, 30-59, 60-99, 100+
    group_names = ['1-9', '10-29', '30-59', '60-99', '99-inf']
    
    result_dict = {}
    for i, name in enumerate(group_names):
        result_dict[f'{name} HR@{topk}'] = HR[i]
        result_dict[f'{name} NDCG@{topk}'] = NDCG[i]
        result_dict[f'{name} Count'] = int(count[i])
