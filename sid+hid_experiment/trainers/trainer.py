...
from models.llm_SASRec import LLM_SASRec, LLM_SASRec_seq
...
class Trainer(object):
    ...
    def _create_model(self):
        ...
        elif self.args.model_name == "LLM_SASRec_seq":
            self.model = LLM_SASRec_seq(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "LLM_SASRec":
            self.model = LLM_SASRec(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "GRU4Rec_seq":
            self.model = GRU4Rec_seq(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "DualLLMSASRec":
            self.model = DualLLMSASRec(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "LLMESR_SASRec":
